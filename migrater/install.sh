...
# Run a command with root privileges when needed.
# Works whether the installer is started as root or a normal user.
if [ "$(id -u)" -eq 0 ]; then
    SUDO=()
else
    if ! command -v sudo >/dev/null 2>&1; then
        die "This installer must be run as root or with sudo installed."
    fi
    SUDO=(sudo)
fi
...
if command -v apt-get >/dev/null 2>&1; then
    ...
    run_root apt-get install -y -qq curl ca-certificates git

elif command -v dnf >/dev/null 2>&1; then
    run_root dnf install -y -q curl ca-certificates git

elif command -v yum >/dev/null 2>&1; then
    run_root yum install -y -q curl ca-certificates git
...
if [ "$NODE_MAJOR" -lt 18 ]; then
    ...
    if command -v apt-get >/dev/null 2>&1; then
        ...
        # Download the NodeSource setup script first.
        NODE_SETUP=$(mktemp)
        ...
        if command -v dnf >/dev/null 2>&1; then
            ...
            die "Could not find dnf or yum to install Node.js."
        fi
    fi
...
if command -v pm2 >/dev/null 2>&1; then
    ...
    run_root npm install -g pm2 --silent
fi
...
if run_root env PATH="$PATH:$NODE_BIN_DIR" \
    pm2 startup systemd \
    -u "$RUN_USER" \
    --hp "$RUN_HOME" >/dev/null 2>&1; then
...
FREE_GB=$(df -BG --output=avail . 2>/dev/null |
    tail -1 |
    tr -dc '0-9' ||
    echo '?')

BUDGET_GB=$(node -p \
    'require("./config").MAX_DISK_BYTES / 1024**3' \
    2>/dev/null ||
    echo '?')
...
if [ "$FREE_GB" != '?' ] &&
   [ "$BUDGET_GB" != '?' ] &&
   [ "$FREE_GB" -lt "$BUDGET_GB" ] 2>/dev/null; then
